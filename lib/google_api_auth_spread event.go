...
type GoogleApiAuthSpreadEvent struct {
	auth_common_methods
}

func (h GoogleApiAuthSpreadEvent) CredentialApi(fileId string, sheetModel *[]model.Event) (string, error) {
	...
	if len(resp.Values) == 0 {
		...
		for _, row := range resp.Values {
			title := row[0].(string)
			body := row[1].(string)
			data := model.Event{Title: title, Date: body}
...
func (h GoogleApiAuthSpreadEvent) SaveToken(path string, authCode string) {
	context.Background()
